...
CREATE TABLE IF NOT EXISTS Project (
  ...
  liveUrl VARCHAR(191),
  repoUrl VARCHAR(191),
  FOREIGN KEY (coverId) REFERENCES File(id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS Testimonial (
  ...
  FOREIGN KEY (avatarId) REFERENCES File(id) ON DELETE CASCADE
);
...
INSERT INTO File (url, alt) VALUES
('https://placehold.co/600x400?text=Project+Cover', 'Project cover placeholder'),
('https://placehold.co/100x100?text=Avatar', 'Avatar placeholder');

SET @coverFile = (SELECT id FROM File WHERE url = 'https://placehold.co/600x400?text=Project+Cover');
SET @avatarFile = (SELECT id FROM File WHERE url = 'https://placehold.co/100x100?text=Avatar');

INSERT INTO Project (slug, title, description, coverId, problem, approach, result, lessons, liveUrl, repoUrl) VALUES
('database-migration', 'Database Migration in France', 'Led GDPR-compliant migration of a medical database for InnoForce EST.', @coverFile, 'Customer wasnt satisfied with the existing database solution.', 'Analysed the current database structure and created a migration plan.', 'Successfully migrated the database with zero downtime and improved performance and customer satisfaction.', 'Thorough documentation and testing are crucial for successful migrations.', NULL, NULL),
('mansion-project', 'Mansion Project', 'Led and coordinated the implementation of a large-scale project infrastructure for a luxury mansion.', @coverFile, 'Customer wished for a complex automation solution.', 'Suggested multiple different approaches, designs and technologies.', 'Successfully implemented a robust automation solution that met all customer requirements.', 'Collaboration and clear communication with stakeholders are key to project success.', NULL, NULL),
('property-management-energy-monitoring', 'Property Management & Energy Monitoring', 'Developed a solution for self-use of solar power and installation of e-mobility', @coverFile, 'Customer wished for a comprehensive energy management solution.', 'Conducted a thorough analysis of energy consumption patterns and proposed tailored solutions.', 'Successfully implemented a system for self-use of solar power and e-mobility integration.', 'Understanding customer needs and providing customized solutions is crucial.', NULL, NULL),
('motocom-app', 'Motocom App', 'Developed an app for motorcycle enthusiasts to track their rides and connect with other riders.', @coverFile, 'No easy combined way to track rides and connect with other riders.', 'Conducted user research to identify key features and developed a user-friendly interface.', 'Still in development.', 'n/a', NULL, NULL),
('multiscreenkiosk-app', 'MultiScreenKiosk App', 'Developed an app for displaying multiple applications and browser instances on a single screen.', @coverFile, 'No easy way to manage and display multiple applications simultaneously for events and presentations.', 'Developed a user-friendly interface for managing multiple applications.', 'Works with multiple additional features implemented and planned.', 'Thorough testing and user feedback are essential for successful multi-app management.', NULL, 'https://github.com/sxwxbxr/MultiScreenKiosk');
...
INSERT INTO Testimonial (quote, name, role, company, avatarId) VALUES
('Seya Weber proved to be a reliable and diligent intern with strong analytical skills: he supported the installation, testing, and migration of medical database solutions and developed an application for the automated processing of patient questionnaires, which is already in productive use at major ENT clinics', 'Christoph Wille', 'CEO', 'InnoForce Est.', @avatarFile),
('Sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.', 'John Smith', 'CTO', 'Tech Corp.', @avatarFile);
